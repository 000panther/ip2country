...
{
	//fields
	public $dir = '/srv/th/htdocs/static/Ip2Country_v3/';
...
/**
		 * method getCountryPerIp
		 *
		 * returns a country code for IP address $ip
		 *
		 * based on the http://software77.net/geo-ip/ database
		 * script reads prepared array files, those are generated
		 * by /static/Ip2Country_v3/createArrayFiles.php and manually
		 * copied to /srv/lib/ip2country_v3/files/
		 *
		 *
		 * @author sigurd
		 * @param string $ip
		 * @return string countrycode corresponding to $ip
		 *
		 */
		public static function getCountryPerIp($originalIp)
		{

			$rangeFilePath = dirname(dirname(__FILE__)) . '/ip2countryFiles/';
			echo "Fetching from $rangeFilePath";
			$defaultCountryCode = 'US';

			//convert the ip to its int representation
			...
			$countryCode = "";
			// if file exists for specified range
			$rangeFilePiecePath = $rangeFilePath . $piece . '.php';
			echo "Trying to include $rangeFilePath";
			if (file_exists($rangeFilePiecePath))
			{
				include $rangeFilePiecePath;
				...
				//Debug_Logger::error("Failed to resolve country code. Range file $rangeFilePiecePath not found.");
				$countryCode = 'no file';
			}

			if(empty($countryCode)) {
				//Debug_Logger::warning("Failed resolving Ip $originalIp, falling back to US");
